...
begin
  ...
  if (EditPesquNome.Text <> '') and (DBEdit1.Text = '') and
    (PanelMostrData.Visible = False) and (ComboBoxEventosButtos.Text <> '') then
  begin
    DM.ZQueryAssistente.Close;
    DM.ZQueryAssistente.Params.Clear;
    DM.ZQueryAssistente.SQL.Clear;
    DM.ZQueryAssistente.SQL.Text :=
      'SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.DISCRIMINACAO, r.NOME_ARQUIVO'
      +' FROM LOG r' + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID ' +
      ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO ' +
      ' left join ACAO ac on r.COD_ACAO = ac.ID ' +
      ' WHERE PO.NOME like ' +
      QuotedStr('%' + EditPesquNome.Text + '%') +
      ' and r.COD_ACAO =' +
      IntToStr(ComboBoxEventosButtos.ItemIndex)
  ...
  if ((EditPesquNome.Text <> '') and (ComboBoxEventosButtos.Text = '') and (PanelMostrData.Visible = False)) then
  begin
    DM.ZQueryAssistente.Close;
    DM.ZQueryAssistente.Params.Clear;
    DM.ZQueryAssistente.SQL.Clear;
    DM.ZQueryAssistente.SQL.Text:='SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.DISCRIMINACAO, r.NOME_ARQUIVO FROM LOG r '
      + ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO '
      + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID '
      + ' left join ACAO ac on r.COD_ACAO = ac.ID '
      + ' WHERE PO.NOME like :nome order by r.INSERDATA, r.HORA desc ';
    DM.ZQueryAssistente.ParamByName('nome').AsString:='%'+EditPesquNome.Text +'%';
    DM.ZQueryAssistente.Open;
    LabelNum.Caption := IntToStr(DM.ZQueryAssistente.RecordCount);
    PanelAlert.Visible := True;
  end;
  ...
  if (DBEdit1.Text <> '') and (PanelMostrData.Visible = False) and
    (EditPesquNome.Text = '') and (ComboBoxEventosButtos.Text <> '') then
  begin
    DM.ZQueryAssistente.Close;
    DM.ZQueryAssistente.Params.Clear;
    DM.ZQueryAssistente.SQL.Clear;
  ...
  begin
    ...
    Finall := DateToStr(DateTimePicker2.Date);
    DM.ZQueryAssistente.SQL.Add(
      'SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.NOME_ARQUIVO, r.DISCRIMINACAO FROM LOG r' + ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO ' + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID ' + ' left join ACAO ac on r.COD_ACAO = ac.ID ' + ' where PO.NOME like ' + QuotedStr('%' +EditPesquNome.Text + '%') + ' and r.INSERDATA >= :inicial and r.INSERDATA <= :finall order by r.INSERDATA, r.HORA desc');
    DM.ZQueryAssistente.ParamByName('inicial').asDate := StrToDate(Inicial);
    DM.ZQueryAssistente.ParamByName('finall').asDate := StrToDate(Finall);
    DM.ZQueryAssistente.Open;
    LabelNum.Caption := IntToStr(DM.ZQueryAssistente.RecordCount);
  ...
  begin
    ...
    Finall := DateToStr(DateTimePicker2.Date);
    DM.ZQueryAssistente.SQL.Add(
      'SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.NOME_ARQUIVO, r.DISCRIMINACAO FROM LOG r' + ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO ' + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID ' + ' left join ACAO ac on r.COD_ACAO = ac.ID ' + ' where r.COD_USUARIO=' + DBEdit1.Text + ' and r.INSERDATA >= :inicial' + ' and r.INSERDATA <= :finall  order by r.INSERDATA, r.HORA desc');
    DM.ZQueryAssistente.ParamByName('inicial').asDate := StrToDate(Inicial);
    DM.ZQueryAssistente.ParamByName('finall').asDate := StrToDate(Finall);
    DM.ZQueryAssistente.Open;
    LabelNum.Caption := IntToStr(DM.ZQueryAssistente.RecordCount);
  ...
  //if (DBEdit1.Text <> '') and (PanelMostrData.Visible = True) and () then
  //begin
  //  DM.ZQueryAssistente.Close;
  //  DM.ZQueryAssistente.Params.Clear;
  //  DM.ZQueryAssistente.SQL.Clear;
