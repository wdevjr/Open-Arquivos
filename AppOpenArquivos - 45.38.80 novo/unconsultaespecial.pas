    ...
    //begin
      try
        ...
        DM.ZQueryRelatorioArquivo.Params[0].Value :=
          DM.sds_view.FieldByName('ID').AsInteger;//selecAtualReport;
...
begin
  ...
  if (EditPesqu.Text <> '') or (PanelMostrData.Visible = True) or
    (DBEdit4.Text <> '') or (DBEdit3.Text <> '') or (DBEdit2.Text <> '') or
    (DBEdit1.Text <> '') then
    begin
    if (FrmLogin.TODOS = 1) then
      begin
        ...
        1:
          ...
          if (CheckBoxConsulta.Checked = True) then
            ...
            if (CodigoUser = '') and (CodigoAssunto <> '') and
              ...
              Dtsrc.DataSet.Filtered := False;
              Dtsrc.DataSet.Filter :=
                ' COD_ASSUNTO=' + CodigoAssunto + ' and COD_USUARIO=' +
                CodigoUserPorNome;
              end;
            if (CodigoUser = '') and (CodigoAssunto <> '') and
              (CodigoUserPorNome = '') and (CodigoAutor <> '') then
        ...
        2:
          ...
          if EditPesqu.Text = '' then
            ...
            if (CheckBoxConsulta.Checked = True) then
              ...
              if (CodigoUser = '') and (CodigoAssunto <> '') and
                (CodigoUserPorNome = '') and (CodigoUserPorNome = '') then
                begin
                Dtsrc.DataSet.Filtered := False;
                Dtsrc.DataSet.Filter := ' COD_ASSUNTO=' + CodigoAssunto;
                end;
