...
type
  ...
  public
    ...
    procedure LoadFromStream(Stream: TStream; Parent: TObject);
      virtual; abstract;

    property Items[Index: integer]: TGIFItem read GetItem
  ...
  protected
    ...
    procedure Warning(Severity: TGIFSeverity; Message: string);
      virtual; abstract;
    procedure BuildHistogram(var Histogram: TColormapHistogram);
      virtual; abstract;
  ...
  public
    ...
    property BackgroundColorIndex: Byte
      read FLogicalScreenDescriptor.BackgroundColorIndex
  ...
  protected
    ...
    class function FindSubExtension(Stream: TStream)
      : TGIFExtensionClass; virtual;
  public
...
{$IFDEF VER12_PLUS} reintroduce; {$ENDIF} virtual;
  ...
  public
    ...
    property Extensions[Index: integer]: TGIFExtension read GetExtension
      write SetExtension; default;
  end;
  ...
  public
    ...
    property CharWidth: Byte index 1 read GetCharWidthHeight
      write SetCharWidthHeight;
    property CharHeight: Byte index 2 read GetCharWidthHeight
      write SetCharWidthHeight;
    property ForegroundColorIndex: Byte index 1 read GetColorIndex
      write SetColorIndex;
    property ForegroundColor: TColor read GetForegroundColor;
    property BackgroundColorIndex: Byte index 2 read GetColorIndex
      write SetColorIndex;
    property BackgroundColor: TColor read GetBackgroundColor;
  ...
  public
    ...
    class function FindSubExtension(Stream: TStream)
      : TGIFExtensionClass; override;
    property Identifier: AnsiString read GetIdentifier write SetIdentifier;
    property Authentication: AnsiString read GetAuthentication
  ...
  public
    procedure LoadFromStream(Stream: TStream; Parent: TObject); override;
    procedure SaveToStream(Stream: TStream); override;
    property SubImages[Index: integer]: TGIFSubImage read GetImage
      write SetImage; default;
  end;
...
{$ENDIF}
  public
    ...
    procedure Warning(Sender: TObject; Severity: TGIFSeverity;
    ...
    property DrawOptions: TGIFDrawOptions read FDrawOptions
      write SetDrawOptions;
...
var
  ...
  GIFImageDefaultDrawOptions: TGIFDrawOptions = [goAsync, goLoop, goTransparent,
...
begin
  ...
  try
    ...
    Result := (GetDIBits(DC, Bitmap, 0,
      abs(TBitmapInfoHeader(BitmapInfo).biHeight), @Bits,
...
begin
  ...
  try
    ...
    DIBFromBit(Stream, Bitmap.Handle, Bitmap.Palette, PixelFormat,
      Header, Bits);
...
{$IFDEF PIXELFORMAT_TOO_SLOW}
  ...
  begin
    ...
    if biHeight > 0 then // bottom-up DIB
      Row := biHeight - Row - 1;
    Result := PAnsiChar(Cardinal(FDIBBits) + Cardinal(Row) * AlignBit(biWidth,
      biBitCount, 32));
  end;
{$ELSE}
  Result := FBitmap.Scanline[Row];
  ...
  try
    ...
    begin
      ...
      if (SrcColors < 256) then
        FillChar(Pointer(longInt(@FDIBInfo^.bmiColors) + sizeof(TRGBQuad) *
          SrcColors)^, 256 - SrcColors, 0);
...
{$ELSE}
    ...
    try
      ...
      FDIB := GDICheck(CreateDIBSection(0 { ScreenDC } , FDIBInfo^,
        DIB_RGB_COLORS, FDIBBits,
...
type
  ...
  public
    ...
    function Lookup(Red, Green, Blue: Byte; var R, g, b: Byte): AnsiChar;
      virtual; abstract;
  ...
  public
    ...
    destructor Destroy; override;
    function Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
      : AnsiChar; override;
  end;
...
function TFastColorLookup.Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
var
  i: integer;
  InverseIndex: integer;
...
begin
  ...
  InverseIndex := (Red SHR 3) OR ((Green AND $F8) SHL 2) OR
    ((Blue AND $F8) SHL 7);
...
// Map color to arbitrary palette
function TSlowColorLookup.Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
begin
  Result := AnsiChar(GetNearestPaletteIndex(FPalette, Red OR (Green SHL 8) OR
    (Blue SHL 16)));
  if (FPaletteEntries <> nil) then
    with FPaletteEntries^[Ord(Result)] do
    begin
...
function TNetscapeColorLookup.Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
begin
  R := (Red + 3) DIV 51;
  g := (Green + 3) DIV 51;
...
function TGrayWindowsLookup.Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
begin
...
function TGrayScaleLookup.Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
begin
...
function TMonochromeLookup.Lookup(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
begin
...
type
  ...
  public
    ...
    function Dither(Red, Green, Blue: Byte; var R, g, b: Byte)
      : AnsiChar; virtual;
  ...
  private
    ErrorsR0, ErrorsG0, ErrorsB0, ErrorsR1, ErrorsG1, ErrorsB1, ErrorsR2,
      ErrorsG2, ErrorsB2: PErrors;
    ErrorR0, ErrorG0, ErrorB0, ErrorR1, ErrorG1, ErrorB1, ErrorR2, ErrorG2,
      ErrorB2: PErrors;
    FDirection2: integer;
  protected
    FDivisor: integer;
  ...
  protected
    procedure Propagate(Errors0, Errors1, Errors2: PErrors;
      Error: integer); override;
  public
    constructor Create(AWidth: integer; Lookup: TColorLookup); override;
  end;
  ...
  private
    ErrorsR0, ErrorsG0, ErrorsB0, ErrorsR1, ErrorsG1, ErrorsB1, ErrorsR2,
      ErrorsG2, ErrorsB2, ErrorsR3, ErrorsG3, ErrorsB3: PErrors;
    ErrorR0, ErrorG0, ErrorB0, ErrorR1, ErrorG1, ErrorB1, ErrorR2, ErrorG2,
...
function TDitherEngine.Dither(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
begin
...
constructor TFloydSteinbergDitherer.Create(AWidth: integer;
  Lookup: TColorLookup);
begin
...
function TFloydSteinbergDitherer.Dither(Red, Green, Blue: Byte;
  var R, g, b: Byte): AnsiChar;
var
...
begin
  ...
  // Map color to palette
  Result := inherited Dither(CurrentErrorR, CurrentErrorG,
    CurrentErrorB, R, g, b);
...
function T5by3Ditherer.Dither(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
var
...
procedure TStuckiDitherer.Propagate(Errors0, Errors1, Errors2: PErrors;
  Error: integer);
begin
  if (Error = 0) then
    exit;
...
procedure TSierraDitherer.Propagate(Errors0, Errors1, Errors2: PErrors;
  Error: integer);
var
  TempError: integer;
begin
...
procedure TJaJuNiDitherer.Propagate(Errors0, Errors1, Errors2: PErrors;
  Error: integer);
var
  TempError: integer;
begin
...
function TSteveArcheDitherer.Dither(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
var
  ColorR, ColorG, ColorB: integer; // Error for current pixel
  ...
  procedure Propagate(Errors0, Errors1, Errors2, Errors3: PErrors;
    Error: integer);
  var
    TempError: integer;
  begin
...
function TBurkesDitherer.Dither(Red, Green, Blue: Byte; var R, g, b: Byte)
  : AnsiChar;
var
...
type
  ...
  protected
    procedure AddColor(var Node: TOctreeNode; R, g, b: Byte; ColorBits: integer;
      Level: integer; var LeafCount: integer;
...
begin
  ...
  begin
    ...
    begin
      with Pixel^ do
        AddColor(FTree, rgbtRed, rgbtGreen, rgbtBlue, FColorBits, 0, FLeafCount,
          FReducibleNodes);
...
begin
  ...
  begin
    ...
    AddColor(Node.Child[Index], R, g, b, ColorBits, Level + 1, LeafCount,
      ReducibleNodes);
  end;
end;
...
begin
  ...
  try
    Result := doCreateOptimizedPaletteFromSingleBitmap(DIB, Colors,
      ColorBits, Windows);
  finally
    DIB.Free;
  end;
...
{$ENDIF}
    ...
    try // Protect above resources
      ...
      case (ColorReduction) of
        ...
        rmQuantizeWindows:
          Palette := CreateOptimizedPaletteFromSingleBitmap(Bitmap,
            256, 8, True);
        rmNetscape:
          Palette := WebPalette;
        rmGrayScale:
      ...
      begin
        ...
        Src := Pointer(longInt(SrcScanLine) + Ditherer.Column *
          sizeof(TRGBTriple));
...
{$IFDEF DEBUG_DITHERPERFORMANCE}
  ...
  ShowMessage
    (format('Dithered %d pixels in %d mS, Rate %d pixels/mS (%d pixels/S)',
    [Bitmap.Height * Bitmap.Width, TimeStop - TimeStart, MulDiv(Bitmap.Height,
    Bitmap.Width, TimeStop - TimeStart + 1), MulDiv(Bitmap.Height,
...
begin
  ...
  if (Index < FCount) then
    System.Move(FColorMap^[Index + 1], FColorMap^[Index],
      (FCount - Index) * sizeof(TGIFColor));
  FOptimized := False;
  Changed;
end;
...
begin
  ...
  begin
    ...
    System.Move(TGIFColorMap(Source).FColorMap^, FColorMap^,
      FCount * sizeof(TGIFColor));
    Changed;
  end
  else
  ...
  *)
  ...
  for i := 0 to FHeader.Image.Images.Count - 1 do
    ...
    begin
      ...
      LastPixel := Pixel + FHeader.Image.Images[i].Width * FHeader.Image.Images
        [i].Height;
      ...
      *)
      if (FHeader.Image.Images[i].Transparent) then
        FHeader.Image.Images[i].GraphicControlExtension.TransparentColorIndex :=
          Map[FHeader.Image.Images[i].GraphicControlExtension.
...
begin
  ...
  if (FLogicalScreenDescriptor.PackedFields AND
...
begin
  ...
  if (FLogicalScreenDescriptor.PackedFields AND
    lsdGlobalColorTable = lsdGlobalColorTable) then
  begin
    ColorCount := 2 SHL (FLogicalScreenDescriptor.PackedFields AND
      lsdColorTableSize);
...
{$ENDIF}
  ...
  const
    masks: array [0 .. 15] of integer = ($0000, $0001, $0003, $0007, $000F,
      $001F, $003F, $007F, $00FF, $01FF, $03FF, $07FF, $0FFF, $1FFF,
      $3FFF, $7FFF);
  var
...
begin
  ASSERT(longInt($FFFFFFFF) = -1,
    'TGIFImage implementation assumes $FFFFFFFF = -1');
...
begin
{$IFDEF DEBUG_HASHPERFORMANCE}
  ShowMessage(format('Found: %d  HitRate: %.2f', [CountLookupFound,
...
{$IFDEF DEBUG_HASHFILLFACTOR}
  ...
  for i := 0 to HashSize - 1 do
    if (HashTable[i] SHR GIFCodeBits <> HashEmpty) then
      inc(Count);
  ShowMessage(format('Size: %d, Filled: %d, Rate %.4f', [HashSize, Count,
    Count / HashSize]));
{$ENDIF}
...
begin
  ...
  while (HashTable[HKey] AND (HashEmpty SHL GIFCodeBits) <>
    (HashEmpty SHL GIFCodeBits)) do { Optimized }
...
type
  ...
  public
    procedure Compress(AStream: TStream; ABitsPerPixel: integer;
      AWidth, AHeight: integer; AInterlace: Boolean; AData: PAnsiChar;
      AMaxColor: integer);
    property Warning: TGIFWarning read FOnWarning write FOnWarning;
  end;
...
const
  BitBucketMask: array [0 .. 16] of longInt = ($0000, $0001, $0003, $0007,
    $000F, $001F, $003F, $007F, $00FF, $01FF, $03FF, $07FF, $0FFF, $1FFF, $3FFF,
    $7FFF, $FFFF);
begin
  if (OutputBits > 0) then
    OutputBucket := (OutputBucket AND BitBucketMask[OutputBits]) OR
      (longInt(Value) SHL OutputBits)
  else
    OutputBucket := Value;
...
type
  ...
  private
    ...
    RunLengthTableMax, RunLengthTablePixel, OutCount, OutClear,
      OutBump: integer;
  protected
...
function TRLEEncoder.ComputeTriangleCount(Count: integer;
  nrepcodes: integer): integer;
var
  PerRepeat: integer;
  n: integer;
...
begin
  ...
  try
    ...
    Encoder.Compress(Stream, BitsPerPixel, Width, Height, Interlaced, FData,
      MaxColors);
  finally
    Encoder.Free;
  end;
...
procedure TGIFExtensionList.SetExtension(Index: integer;
...
begin
  case (Index) of
    1:
      DoSetBounds(Value, FImageDescriptor.Top, FImageDescriptor.Width,
        FImageDescriptor.Height);
    2:
      DoSetBounds(FImageDescriptor.Left, Value, FImageDescriptor.Width,
        FImageDescriptor.Height);
    3:
      DoSetBounds(FImageDescriptor.Left, FImageDescriptor.Top, Value,
        FImageDescriptor.Height);
    4:
    ...
    try // Protect above resources
      ...
      begin
        ...
        MappedTransparentIndex :=
          AnsiChar(Color.Blue DIV 51 + MulDiv(6, Color.Green, 51) + MulDiv(36,
          Color.Red, 51) + 1);
      end;
      ...
      try
        ...
        begin
          if ((Row AND $1F) = 0) then
            Image.Progress(self, psRunning, MulDiv(Row, 100, Height), False,
              Rect(0, 0, 0, 0), sProgressRendering);
...
{$IFDEF DEBUG_RENDERPERFORMANCE}
  ...
  begin
    ShowMessage
      (format('Render %d images in %d mS, Rate %d mS/image (%d images/S)',
      [ImageCount, RenderTime, RenderTime DIV (ImageCount + 1),
...
{$ENDIF}
  try
    Image.Progress(self, psStarting, 0, False, Rect(0, 0, 0, 0),
      sProgressRendering);
    try
...
begin
  Result := Rect(FImageDescriptor.Left, FImageDescriptor.Top,
    FImageDescriptor.Left + FImageDescriptor.Width, FImageDescriptor.Top +
    FImageDescriptor.Height);
end;
...
var
  ...
  Zap: Boolean;
begin
  Zap := (FImageDescriptor.Width <> Width) or
...
begin
  DoSetBounds(Value.Left, Value.Top, Value.Right - Value.Left + 1,
...
begin
  ...
  begin
    ...
    if (ColorMap.Optimized) then
      Pack := Pack OR idSort;
    Pack := (Pack AND NOT(idColorTableSize)) OR
      (ColorResolution AND idColorTableSize);
  end
  else
...
begin
  ...
  if (FImageDescriptor.PackedFields AND idLocalColorTable = idLocalColorTable)
  then
  begin
...
{$ENDIF}
  ...
  begin
    ...
    try
      ...
      Mask.Handle := Windows.CopyImage(hMask, IMAGE_BITMAP, 0, 0,
        LR_COPYRETURNORG); // 2003.08.04
      AddMask(Mask);
    finally
        ...
        try
          ...
          try
            ...
            try
              ...
              // OrMask := NOT Mask
              BitBlt(OrMaskDC, SrcX, SrcY, SrcW, SrcH, MaskDC, SrcX, SrcY,
                NotSrcCopy);
              ...
              // Mem := OrMask
              BitBlt(MemDC, SrcX, SrcY, SrcW, SrcH, OrMaskDC, SrcX,
                SrcY, SrcCopy);
              // Mem := Mem AND Src
...
{$ELSE}
              StretchBlt(DstDC, DstX, DstY, DstW DIV 2, DstH, MemDC, SrcX, SrcY,
                SrcW, SrcH, SrcCopy);
              StretchBlt(DstDC, DstX + DstW DIV 2, DstY, DstW DIV 2, DstH,
...
{$ENDIF}
        ...
        // Neat trick!
        // Dst := Dst AND Mask
        StretchBlt(DstDC, DstX, DstY, DstW, DstH, MaskDC, SrcX, SrcY, SrcW,
          SrcH, SrcAnd);
        // Dst := Dst OR Mem
        StretchBlt(DstDC, DstX, DstY, DstW, DstH, MemDC, SrcX, SrcY, SrcW, SrcH,
          SrcPaint);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            TransparentStretchBlt(ACanvas.Handle, Tile.Left, Tile.Top, Width,
              Height, Bitmap.Canvas.Handle, 0, 0, Width, Height, MaskDC, 0, 0);
...
{$ENDIF}
        ...
        TransparentStretchBlt(ACanvas.Handle, Rect.Left, Rect.Top,
          Rect.Right - Rect.Left, Rect.Bottom - Rect.Top, Bitmap.Canvas.Handle,
...
{$IFDEF DEBUG_DRAWPERFORMANCE}
  ...
  begin
    TimeStop := timeGetTime;
    ShowMessage
      (format('Draw %d images in %d mS, Rate %d images/mS (%d images/S)',
      [ImageCount, TimeStop - TimeStart, ImageCount DIV (TimeStop - TimeStart +
      1), MulDiv(ImageCount, 1000, TimeStop - TimeStart + 1)]));
  end;
{$ENDIF}
end;
    ...
    begin
      ...
      for i := 0 to Image.Images.Count - 1 do
        ...
        begin
          ...
          Result := Image.Images[i].GraphicControlExtension.
            TransparentColorIndex;
          exit;
        end;
...
begin
  ...
  if (Empty) or (Previous = nil) or (Previous.Empty) or
    ...
    (Previous.GraphicControlExtension.Disposal in [dmBackground, dmPrevious]))
  then
    exit;
  ...
  begin
    pSource := PAnsiChar(integer(Previous.Scanline[PreviousY]) + MergeRect.Left
      - Previous.Left);
    ...
    begin
      ...
      if (not(((not NeedTransparentColorIndex) and (pDest^ = AnsiChar(DestIndex)
        )) or ((SourceTransparent) and
        (pSource^ = AnsiChar(SourceIndex))))) and (
...
class function TGIFExtension.FindSubExtension(Stream: TStream)
  : TGIFExtensionClass;
begin
  Result := self;
end;
...
begin
  ...
  if (Value) then
    ...
    FGCExtension.PackedFields := FGCExtension.PackedFields AND
      NOT(efTransparent);
end;
...
begin
  FGCExtension.TransparentColorIndex :=
    SubImage.ActiveColorMap.AddUnique(Color);
end;
...
begin
  FGCExtension.PackedFields := FGCExtension.PackedFields AND NOT(efDisposal) OR
    ((Ord(Value) SHL 2) AND efDisposal);
end;
...
begin
  ...
  if (Stream.Read(FGCExtension, sizeof(FGCExtension)) <> sizeof(FGCExtension))
  then
  begin
    Warning(gsWarning, sOutOfData);
    exit;
...
const
  NSLoopIdent: array [0 .. 1] of TGIFApplicationRec = ((Identifier: 'NETSCAPE';
    Authentication: '2.0'), (Identifier: 'ANIMEXTS'; Authentication: '1.0'));
begin
  inherited Create;
...
function TAppExtensionList.FindExt(eIdent: TGIFApplicationRec)
...
class procedure TGIFApplicationExtension.RegisterExtension
  (eIdent: TGIFApplicationRec; eClass: TGIFAppExtensionClass);
begin
...
begin
  ...
  StrLCopy(@(FIdent.Authentication[0]), PAnsiChar(Value),
    sizeof(TGIFAuthenticationCode));
end;
...
begin
  ...
  StrLCopy(@(FIdent.Identifier[0]), PAnsiChar(Value),
    sizeof(TGIFIdentifierCode));
end;
...
const
  NSLoopIdent: TGIFApplicationRec = (Identifier: 'NETSCAPE';
    Authentication: '2.0');
begin
  inherited Create(ASubImage);
  FIdent := NSLoopIdent;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        Add(SubImage);
        Image.Progress(self, psRunning, MulDiv(Stream.Position, 100,
          Stream.Size), GIFImageRenderOnLoad, Rect(0, 0, 0, 0),
          sProgressLoading);
      except
        SubImage.Free;
        raise;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        BackupBuffer.Canvas.FillRect(FImage.Images[ActiveImage]
          .ScaleRect(FRect));
    end;
  end;
...
begin
  ...
  if not((FImage.Images[ActiveImage].Empty) or
    (FImage.Images[ActiveImage].HasBitmap)) then
    ...
    try
      ...
      begin
        ...
        begin
          ...
          // Loop to animate
          while (ActiveImage < FImage.Images.Count) and
            not(Terminated or DoRestart) do
          begin
            // Ignore empty images
...
{$ENDIF}
              ...
              begin
                ...
                try
                  ...
                  Delay := MulDiv(Delay * GIFDelayExp - DelayUsed, 100,
                    FAnimationSpeed);
                  OldDelay := Delay;
                except
...
{$ENDIF}
            ...
            if (FImage.Images[ActiveImage].GraphicControlExtension <> nil) then
              if (FImage.Images[ActiveImage].GraphicControlExtension.Delay > 0)
              then
              begin
                Delay := FImage.Images[ActiveImage]
...
type
  ...
  public
    ...
    procedure MapImages(UseTransparency: Boolean;
      NewTransparentColorIndex: Byte);
    property Count: integer read FCount;
    property ColorMap: TGIFColorMap read FColorMap;
...
begin
  Result := POptimizeEntry(Item2)^.Color.Value - POptimizeEntry(Item1)
    ^.Color.Value;
end;
...
begin
  ...
  if (IsTransparent) then
    OldTransparentColorIndex := Image.GraphicControlExtension.
      TransparentColorIndex
  else
...
procedure THistogram.MapImages(UseTransparency: Boolean;
  NewTransparentColorIndex: Byte);
var
  i: integer;
  Size: integer;
...
begin
  ...
  *)
  for i := 0 to List.Count - 1 do
    ReverseMap[POptimizeEntry(List[i])^.OldIndex] := POptimizeEntry(List[i])
      ^.NewIndex;
...
begin
  ...
  *)
  ...
  begin
    ...
    for i := 0 to FImage.Images.Count - 1 do
      if (FImage.Images[i].ColorMap.Count = 0) and (not FImage.Images[i].Empty)
      then
      begin
        ProcessedImage := True;
        // Note: Do not change order of statements. Shortcircuit evaluation not desired!
        FUseTransparency := Hist.ProcessSubImage(FImage.Images[i]) or
          FUseTransparency;
      end;
  ...
  for i := FFinalCount to FHistogram.Count - 1 do
    ...
    begin
      ...
      begin
        ...
        ASSERT(Color.SameAs.NewIndex < 256,
          'Internal error: Mapping to unmapped color');
      ...
      begin
        ...
        begin
          Delta := abs((POptimizeEntry(FHistogram[j])^.Color.Color.Red -
            ...
            - Color.Color.Green) + (POptimizeEntry(FHistogram[j])
...
var
  ...
  function Scan(Buf: PAnsiChar; Value: Byte; Count: integer): Boolean;
    assembler;
  asm
    PUSH	EDI
    MOV		EDI, Buf
    ...
    try
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if (Images[i].GraphicControlExtension <> nil) and
              (Images[i].GraphicControlExtension.Disposal in [dmPrevious,
              dmBackground]) and (not Images[i + 1].Transparent) then
            begin
          ...
          begin
            ...
            begin
              ...
              if ((GCE.Delay = 0) or (Images.Count = 1)) and
                ...
                ((GCE.Disposal in [dmNone, dmNoDisposal]) or (Images.Count = 1))
              then
              begin
                GCE.Free;
              end;
        ...
        if ExceptObject = nil then
          ...
          i := 0;
        Progress(self, psEnding, i, False, Rect(0, 0, 0, 0),
          sProgressOptimizing);
      end;
    finally
      FPainters.UnlockList;
...
procedure TGIFImage.LoadFromResourceName(Instance: THandle;
...
begin
  if (GIFImageOptimizeOnStream) then
    Optimize([ooCrop, ooMerge, ooCleanup, ooColorMap, ooReduceColors], rmNone,
      dmNearest, 8);
      ...
      try
        ...
        try
          ...
          begin
            ...
            Add(Image);
            Progress(self, psRunning, MulDiv((i + 1), 100,
              TGIFImage(Source).Images.Count), False, Rect(0, 0, 0, 0),
              sProgressCopying);
          end;
{$IFDEF FIXHEADER_WIDTHHEIGHT_SILENT}
          FixHeaderWidthHeight; // 2006.07.09
...
{$ELSE}
    ...
    while PeekMessage(Msg, ThreadWindow, CM_DESTROYWINDOW, CM_EXECPROC,
      PM_REMOVE) do
    begin
      if (Msg.Message <> WM_QUIT) then
      begin
  ...
  try
    ...
    begin
      ...
      if (FForceFrame >= 0) and (FForceFrame < FImages.Count) then
        FImages[FForceFrame].Draw(ACanvas, Rect,
          (goTransparent in FDrawOptions), (goTile in FDrawOptions))
      else
        // 2004.03.09 <-
...
{$ELSE}
            ...
            while (FDrawPainter <> nil) and (not FDrawPainter.Terminated) and
              ...
              // Process Messages to make Synchronize work
              // (Instead of Application.ProcessMessages)
              if PeekMessage(Msg, ThreadWindow, CM_DESTROYWINDOW, CM_EXECPROC,
                PM_REMOVE) then
              begin
                if (Msg.Message <> WM_QUIT) then
                begin
...
procedure TGIFImage.Warning(Sender: TObject; Severity: TGIFSeverity;
  Message: string);
begin
