...
uses
  ...
  Menus, TplGifAnimatorUnit, GifAnim, DateUtils, WinInet, ATGauge;
  ...
  if FrmDlgPesquisa.ShowModal = mrOk then
    if (FrmLogin.TODOS = 0) then
      begin
        ...
        frmSplash.Show;
        frmSplash.Refresh;
        //frmSplash.plGifAnimator1.Animate:=True;
...
begin
  ...
  if (DBText2.Caption = '') then
    ...
    if (Verifica_Campos_Em_Branco) then
      ...
      frmSplash.Refresh;
      frmSplash.ProgressBar1.Position := 1;
      plGifAnimator1.Visible := True;
      plGifAnimator1.Animate := True;
      // espera...
      Sleep(100);
      // altera a ProgressBar
      frmSplash.ProgressBar1.Position := 8;
      ...
      Screen.Cursor := crHourGlass;
        ...
        try
          ...
          DM.cds_logNOME_ARQUIVO.Value := StrTempCodRegist;
      ...
      if frmSplash.ProgressBar1.Position = 100 then
        begin
        plGifAnimator1.Animate := False;
        plGifAnimator1.Visible := False;
        frmSplash.Hide;
        frmSplash.Free;
        end;
      end
...
var
  IntTempCodResgist: integer;
  StrTempNomeArquivo: string;
begin
  IntTempCodResgist := 0;
  StrTempNomeArquivo := '';
  IntTempCodResgist := DM.ZQArquivoID.AsInteger;
  StrTempNomeArquivo := DM.ZQArquivoNOME_ARQUIVO.AsString;
    try
      begin
      //if not DM.cds_Log.Active then
      //begin
      DM.cds_Log.Open;
      DM.cds_Log.Append;
      //end else
      ...
      DM.cds_LogAPLICATIVO.Value := 'CADASTRO DE ARQUIVOS';
      DM.cds_LogCOD_REGISTRO.Value := IntTempCodResgist;
      DM.cds_logNOME_ARQUIVO.Value := StrTempNomeArquivo;
  ...
  if (DBMemo1.Text = '') then
    ...
    if (DBText2.Caption = '') then
      ...
      if Verifica_Campos_Em_Branco then
        ...
        if frmSplash.ProgressBar1.Position = 100 then
          begin
          //frmSplash.plGifAnimator1.Animate:=False;
          frmSplash.Hide;
          frmSplash.Free;
          end;
        end
