...
begin
  ...
  if ((EditPesquNome.Text <> '') and (ComboBoxEventosButtos.Text = '') and
    ...
    DM.ZQueryAssistente.SQL.Clear;
    DM.ZQueryAssistente.SQL.Text :=
      'SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.DISCRIMINACAO, r.NOME_ARQUIVO FROM LOG r ' + ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO ' + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID ' + ' left join ACAO ac on r.COD_ACAO = ac.ID ' + ' WHERE PO.NOME like :nome order by r.INSERDATA, r.HORA desc ';
  ...
  if (EditPesquNome.Text = '') and (DBEdit1.Text <> '') and
    ...
    DM.ZQueryAssistente.SQL.Clear;
    DM.ZQueryAssistente.SQL.Text :=
      'SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.DISCRIMINACAO, r.NOME_ARQUIVO' + ' FROM LOG r' + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID ' + ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO ' + ' left join ACAO ac on r.COD_ACAO = ac.ID ' + ' WHERE PO.NOME like ' + QuotedStr('%' + EditPesquNome.Text + '%')
  ...
  if (DBEdit1.Text <> '') and (PanelMostrData.Visible = False) and
    ...
    DM.ZQueryAssistente.SQL.Text :=
      ...
      DBEdit1.Text + ' and r.COD_ACAO =' +
      IntToStr(ComboBoxEventosButtos.ItemIndex) + ' order by r.INSERDATA, r.HORA desc';
  ...
  if (DBEdit1.Text <> '') and (PanelMostrData.Visible = False) and
    ...
    //Finall := DateToStr(DateTimePicker2.Date);
    DM.ZQueryAssistente.SQL.Add(
      'SELECT r.CODIGO,r.INSERDATA, r.HORA,r.COD_USUARIO, r.COD_ACAO, r.ACAO, PO.NOME,r.APLICATIVO, r.COD_REGISTRO, r.DISCRIMINACAO, r.NOME_ARQUIVO FROM LOG r ' + ' left join USUARIO PO on r.COD_USUARIO = PO.COD_USUARIO ' + ' left join ARQUIVOLIST ar on r.COD_REGISTRO = ar.ID ' + ' left join ACAO ac on r.COD_ACAO = ac.ID ' + ' where r.COD_USUARIO=' + DBEdit1.Text + ' order by r.INSERDATA, r.HORA desc');
